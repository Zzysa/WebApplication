...
app.use("/api/users", async (req, res, next) => {
  try {
    ...
    if (response.status === 304) {
      return res
        .status(200)
        .json({ message: "User data not modified", cached: true });
    }

    res.status(response.status).json(response.data);
  } catch (error) {
    console.error(
      "[API Gateway] Users error:",
...
app.use("/api/admin", checkAdmin, async (req, res, next) => {
  try {
    let targetUrl;
    if (req.originalUrl.startsWith("/api/admin/coupons")) {
      targetUrl = PRODUCT_SERVICE_URL;
    } else {
      targetUrl = USER_SERVICE_URL;
    }
